...
const defaultConfig: AxiosRequestConfig = {
    // 请求超时时间
    timeout: 10000,
    // 后端 API 基础路径（开发走 vite proxy /api → :8080）
    ...
    private httpInterceptorsResponse(): void {
        ...
        instance.interceptors.response.use(
            (response: PureHttpResponse) => {
                ...
                // 后端统一响应 {code,message,data}：code!==200 视为业务失败，弹错并 reject
                const res = response.data;
                if (res && res.code !== 200) {
                    // 刷新/登录接口的业务失败交由调用方 catch 统一处理，避免与登出提示双重弹错
                    const whiteList = ["/refresh-token", "/login"];
                    if (!whiteList.some(u => $config.url?.endsWith(u))) {
            ...
            (error: PureHttpError) => {
                ...
                // HTTP 401：服务端 accessToken 失效（Sa-Token NotLoginException），
                // 用 refreshToken 兜底刷新并重放请求，避免用户在 accessToken 过期/服务重启后被直接踢下线
                const status = $error?.response?.status;
                const config = $error?.config as PureHttpRequestConfig | undefined;
                const whiteList = ["/refresh-token", "/login"];
                const isWhite =
                    !!config?.url && whiteList.some(u => config.url.endsWith(u));
                if (status === 401 && config && !isWhite) {
                    if (PureHttp.isRefreshing) {
                        // 并发 401：当前请求入队，待首请求刷新完成后统一重放
                        return new Promise(resolve => {
                            PureHttp.requests.push((token: string) => {
                                config.headers["Authorization"] = formatToken(token);
