...
`vue-pure-admin` is an open source, free and out-of-the-box middle and backend management system template. Completely
adopts `ECMAScript` module (`ESM`) specifications to write and organize code, using the latest `Vue3`, `Vite`,
`Element-Plus`, `TypeScript`, `Pinia`, `Tailwindcss` and other mainstream technologies develop
...
1. Customize the image named `vue-pure-admin` (please note that there is a dot `.` at the end of the command below,
...
2. Port mapping and start the `docker` container (`8080:80`: indicates that the `80` port is used in the container, and
   the port is forwarded to the `8080` port of the host; `pure-admin`: indicates a custom container name;
   `vue-pure-admin`: indicates the custom image name)
...
You are very welcome to join！[Raise an issue](https://github.com/pure-admin/vue-pure-admin/issues/new/choose) Or submit
a Pull Request
...
Thank you very much for your in-depth understanding of the source code and your outstanding contributions to the
...
In actual use, I feel that `Firefox` is smoother in animation than other browsers, but the author is used to using
`Chrome`. It depends on personal preference.  
